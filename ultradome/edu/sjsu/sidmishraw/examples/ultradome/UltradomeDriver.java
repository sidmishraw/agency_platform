...
public class UltradomeDriver {
	...
	public static void main(String[] args) {
		...
		UltradomeFactory ultradomeFactory = InstanceFactory
				.getInstance(UltradomeFactory.class);
		
		// add weapons, shield skins and medicine bowls to the ultradome world
		ultradome.setParameter(UltradomeFactory.WEAPONS,
				ultradomeFactory.makeWorldWeapons());
		ultradome.setParameter(UltradomeFactory.SHIELD_SKINS,
				ultradomeFactory.makeWorldShieldSkins());
		ultradome.setParameter(UltradomeFactory.MEDICINE_BOWLS,
				ultradomeFactory.makeWorldMedicineBowls());
		...
		for (int counter = 1; counter <= MAX_GLADIATORS; counter++) {
			...
			Agent<Strike> gladiator = agentFactory.makeAgent(
					"Gladiator#" + String.valueOf(counter), Strike.class,
					new Parameter("health", MAX_HP),
					new Parameter("weapon",
							ultradomeFactory.makeWeapon("Baby Sword", WeaponType.SWORD,
									() -> {
										
										Strike strike = new Strike(
												"Baby sword strike from hell!!!", 0.01F,
			...
			agentFactory.addBehaviors(gladiator,
					new Parameter("defend", new BiConsumer<Agent<Strike>, Strike>() {
						
						/**
						 * The gladiator accepts the incoming strike and updates
						 * his
						 * health
						 * after reducing the damage and recalculating the
						 * incoming
						 * strike strength
						 * as it passes through the various shield skins adorned
						 * by the
						 * his shield.
						 * 
						 */
						...
						public void accept(Agent<Strike> me, Strike t) {
							...
							synchronized (me) {
								...
								// ask the shield to reduce the strike strength
								// before
								// dealing dmg
								...
								// reduce the health according to the modified
								// dmg,
								// taking
								...
								System.out.println(
										"Gladiator: " + me.getDescription()
												+ " has defended against "
												+ newStrike.getDescription()
												+ " of strength = "
												+ newStrike.getStrength()
												+ ", new HP = "
												+ me.getParameter(Float.class, "health"));
							}
						}
					}),
					new Parameter("strengthenShield",
							new BiConsumer<Agent<Strike>, ShieldSkin>() {
								
								/**
								 * The gladiator accepts a new shield skin from
								 * the world and
								 * applies it on his shield
								 */
								@Override
								public void accept(Agent<Strike> me,
										ShieldSkin shieldSkin) {
									
									synchronized (me) {
										
										Shield shield = me.getParameter(Shield.class,
												"shield");
										...
										System.out
												.println("Gladiator: "
														+ me.getDescription()
														+ " has added a new skin to the shield!");
									}
								}
							}),
					new Parameter("interact",
							new BiConsumer<Agent<Strike>, Agent<Strike>>() {
								
								/**
								 * Interacts with other gladiators, makes a
								 * strike and sends it
								 * in.
								 */
								@Override
								public void accept(Agent<Strike> me,
										Agent<Strike> myPartner) {
									
									synchronized (me) {
										
										// don't know why myPartner was included
										// into the mix,
										// but
										// whatever
										Strike myStrike = me
												.getParameter(Weapon.class, "weapon")
												.makeStrike();
										
										me.getFacilitator().sendMessage(me,
												new Message<>(myStrike));
									}
								}
							}));
				...
				synchronized (gladiator) {
					...
					while (null != (msg = gladiator.getMailbox().poll())) {
						...
						if (null != msg) {
							...
							((BiConsumer<Agent<Strike>, Strike>) gladiator
									.getParameter(BiConsumer.class, "defend"))
											.accept(gladiator, strike);
							
							float currentHealth = gladiator.getParameter(Float.class,
									"health");
							...
							if (currentHealth < 0.0F
								...
								if (gladiator.getFacilitator().amIAlone(gladiator)) {
									...
									System.out.println(
											"Gladiator: " + gladiator.getDescription()
													+ " declares himself as the WINNER from hell $_$!");
								}
								...
								System.out.println(
										"Gladiator: " + gladiator.getDescription()
												+ " has died with final HP of = "
												+ gladiator.getParameter(Float.class,
														"health"));
					...
					System.out
							.println("Gladitor: " + gladiator.getDescription()
									+ " has luck = " + luck);
					
					if (0 <= luck && luck < 10) {
						
						// drink medicine
						MedicineBowl medicine = ((ConcurrentLinkedQueue<MedicineBowl>) gladiator
								.getFacilitator()
								.getParameter(ConcurrentLinkedQueue.class,
										UltradomeFactory.MEDICINE_BOWLS)).poll();
						...
						if (null != medicine) {
							...
							while (medicine.getContents() != 0
								...
								System.out.println(
										"Healing gladiator: " + gladiator.getDescription()
												+ " by +1");
								
								float incrementHealth = gladiator
										.getParameter(Float.class, "health") + 1;
							...
							if (medicine.getContents() > 0) {
								...
								// to
								// the world
								((ConcurrentLinkedQueue<MedicineBowl>) gladiator
										.getFacilitator()
										.getParameter(ConcurrentLinkedQueue.class,
												UltradomeFactory.MEDICINE_BOWLS))
														.add(medicine);
								
								System.out.println(
										"Medicine bowl now has " + medicine.getContents()
						...
						Weapon weapon = ((ConcurrentLinkedQueue<Weapon>) gladiator
								.getFacilitator()
								.getParameter(ConcurrentLinkedQueue.class,
										UltradomeFactory.WEAPONS))
												.poll();
						
						if (null != weapon) {
							
							Weapon oldWeapon = gladiator.getParameter(Weapon.class,
									"weapon");
							
							System.out.println(
									"Switched weapon for gladiator: "
											+ gladiator.getDescription() + " to "
											+ weapon.getName() + " from "
											+ oldWeapon.getName());
							...
							System.out.println(oldWeapon.getName()
									+ " has been reclaimed by Ultradome from Gladiator: "
									+ gladiator.getDescription());
						}
					} else if (20 <= luck && luck <= 30) {
						
						ShieldSkin shieldSkin = ((ConcurrentLinkedQueue<ShieldSkin>) gladiator
								.getFacilitator()
								.getParameter(ConcurrentLinkedQueue.class,
										UltradomeFactory.SHIELD_SKINS))
												.poll();
						
						if (null != shieldSkin) {
							
							System.out.println("Applying new shield skin for gladiator: "
									+ gladiator.getDescription());
							
							((BiConsumer<Agent<Strike>, ShieldSkin>) gladiator
									.getParameter(
											BiConsumer.class,
											"strengthenShield")).accept(gladiator,
													shieldSkin);
							
							System.out.println("Now gladiator: "
									+ gladiator.getDescription() + " has "
									+ gladiator.getParameter(Shield.class, "shield")
											.nbrSkins()
									+ " skins on his shield!");
						}
					} else {
						...
						System.out.println(
								"Gladiator: " + gladiator.getDescription()
										...
										+ ((gladiator.getPartner() == null) ? "No one"
												: gladiator.getPartner()
														.getDescription()));
						
						// send a strike to the partner
						if (null != gladiator.getPartner()) {
							
							((BiConsumer<Agent<Strike>, Agent<Strike>>) gladiator
									.getParameter(
											BiConsumer.class,
											"interact")).accept(gladiator,
													gladiator.getPartner());
							
							if (null != gladiator.getPartner()) {
								
								System.out.println(
										"Strike sent by " + gladiator.getDescription()
												+ ", so dropping the partner "
												+ gladiator.getPartner()
														.getDescription());
							}
						}
